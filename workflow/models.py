...
class Workflow(BaseModel):
    ...
    notices = models.CharField(
        ...
        blank=True,
        max_length=50,
        help_text="CustomNotice中的id.逗号隔开多个通知方式",
    )
    ...
    limit_expression = models.CharField(
        ...
        blank=True,
        help_text=(
            "限制周期({\"period\":24} 24小时), 限制次数({\"count\":1}在限制周期内只允许提交1次), "
            "限制级别({\"level\":1} 针对(1单个用户 2全局)限制周期限制次数,默认特定用户);允许特定人员提交({\"allow_persons\":\"zhangsan,lisi\"}只允许张三提交工单,"  # noqa: E501,B950
            "{\"allow_depts\":\"1,2\"}只允许部门id为1和2的用户提交工单，{\"allow_roles\":\"1,2\"}只允许角色id为1和2的用户提交工单)"
        ),
    )
    display_form_str = models.CharField(
        ...
        blank=True,
        help_text=(
            "默认\"[]\"，用于用户只有对应工单查看权限时显示哪些字段,field_key的list的json,如[\"days\",\"sn\"],"
    ...
    content_template = models.CharField(
        "内容模板",
        max_length=1000,
        default="标题:{title}, 创建时间:{gmt_created}",
        null=True,
        blank=True,
        help_text="工单字段的值可以作为参数写到模板中，格式如：标题:{title}, 创建时间:{gmt_created}",
    )
...
class WorkflowAdmin(BaseModel):
    ...
    username = models.CharField(
        "管理员",
        max_length=100,
        help_text="除超级管理员及该工作流创建人外，管理人员也可以直接编辑该工作流",
    )
...
class State(BaseModel):
    ...
    participant_type_id = models.IntegerField(
        ...
        default=1,
        blank=True,
        help_text="0.无处理人,1.个人,2.多人,3.部门,4.角色,5.变量(支持工单创建人,创建人的leader),6.脚本(已废弃),7.工单的字段内容(如表单中的\"测试负责人\"，需要为用户名或者逗号隔开的多个用户名),8.父工单的字段内容,10.hook,11.外部获取。 初始状态请选择类型5，参与人填creator",
    )
    ...
    state_field_str = models.TextField(
        "表单字段",
        default="{}",
        help_text="json格式字典存储,包括读写属性1：只读，2：必填，3：可选. 示例：{\"created_at\":1,\"title\":2, \"sn\":1}, 内置特殊字段participant_info.participant_name:当前处理人信息(部门名称、角色名称)，state.state_name:当前状态的状态名,workflow.workflow_name:工作流名称",
    )
...
class Transition(BaseModel):
    ...
    transition_type_id = models.IntegerField(
        "流转类型",
        default=1,
        help_text="1.常规流转，2.定时器流转,需要设置定时器时间",
    )
    timer = models.IntegerField(
        "定时器(单位秒)",
        default=0,
        help_text="流转类型设置为定时器流转时生效,单位秒。处于源状态X秒后如果状态都没有过变化则自动流转到目标状态",
    )
